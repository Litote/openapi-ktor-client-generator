...
public class OpenApiSpecificationParser(
    ...
    override fun parse(operationFilter: (OperationMeta) -> Boolean): GenerationSpec {
    ...
    private fun adjustTypeForAdditionalModel(
        ...
        val isEnum = additionalModel is ModelSpec.EnumSpec
        val inlineType = DomainType.InlineType(additionalTypeName, isEnum = isEnum)
        return when (domainType) {
            is DomainType.ListType -> domainType.copy(element = inlineType)
            is DomainType.SetType -> domainType.copy(element = inlineType)
            is DomainType.InlineType -> inlineType
            else -> domainType
        }
    }
    ...
    private fun buildRequestBodySpec(
        ...
        if (isMultipartFormData || isUrlEncodedForm) {
            ...
            val domainType = DomainType.InlineType("${operationName}Form")
        ...
        val requestType =
            when {
                inlineObjectSchema != null -> {
                    ...
                    DomainType.InlineType("${operationName}Request")
                }
    ...
    private fun buildFormFields(
        ...
        return formSchema.properties
            ?.map { (name, propertySchema) ->
                ...
                val fieldType =
                    if (isBinary) {
                        DomainType.InlineType(fileClassName)
                    } else {
    ...
    private fun buildModelProperties(
        ...
        return rawProperties.map { property ->
            ...
            if (propSchema != null && propSchema.firstType == OpenAPIV3Type.OBJECT &&
                ...
                val inlineType = DomainType.InlineType(nestedName, isEnum = false)
    ...
    private fun buildDefaultValueFromCodeBlock(
        ...
        return when {
            typeName.isString() -> {
                DefaultValue.StringDefault(str.removeSurrounding("\""))
            }

            typeName.isBoolean() -> {
                str.toBooleanStrictOrNull()?.let { DefaultValue.BooleanDefault(it) }
            }

            typeName.isLong() -> {
                str.toLongOrNull()?.let { DefaultValue.LongDefault(it) }
            }

            typeName.isDouble() -> {
                str.toDoubleOrNull()?.let { DefaultValue.DoubleDefault(it) }
            }

            typeName.isFloat() -> {
                str.removeSuffix("F").toFloatOrNull()?.let { DefaultValue.FloatDefault(it) }
            }

            typeName.isInt() -> {
                str.toIntOrNull()?.let { DefaultValue.IntDefault(it) }
            }
                ...
                val parts = str.split(".")
                if (parts.size == 2) {
                    DefaultValue.EnumDefault(typeName = parts[0], enumValue = parts[1])
                } else {
                    null
                }
            }
