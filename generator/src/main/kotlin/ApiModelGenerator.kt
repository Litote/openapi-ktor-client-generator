...
public class ApiModelGenerator internal constructor(public val apiModel: ApiModel) {
    ...
    internal fun buildModel(name: String, schema: OpenAPIV3Schema): TypeSpec {
        ...
        return if (properties.isEmpty()) {
            ...
            TypeSpec.classBuilder(name)
                            ...
                            addParameter(
                                ParameterSpec
                                    ...
                                    .apply {
                                        ...
                                        (when {
                                            property.asSchema?.default != null -> (property.asSchema?.default as? JsonPrimitive)?.content?.let {
                                                if (it == "null" || !isEnum) {
                                                    it
                                                } else {
                                            ...
                                            else -> if (isEnum && defaultEnumValue != null) "${(property.type as ClassName).simpleName}.$defaultEnumValue" else null
                                        })?.apply {
                                            val format = if (this != "null") {
                ...
                .apply {
                    properties.forEach { property ->
                        addProperty(
                            PropertySpec.builder(property.camelCaseName, property.type)
                                .initializer(property.camelCaseName)
                                .apply {
                                    if (property.isSnakeCase) {
                                        addAnnotation(
                                            AnnotationSpec.builder(SerialName::class)
                                                .addMember("%S", property.initialName).build()
                                        )
                                    }
                                }
                                .build()
                        ...
                        if (!property.asSchema?.enum.isNullOrEmpty()) {
                            addType(
                                TypeSpec.enumBuilder(property.camelCaseName.capitalize()).apply {
    ...
    internal fun writeFile(name: String, typeSpec: TypeSpec) {
        ...
        println("Writing $name to $basePath")
